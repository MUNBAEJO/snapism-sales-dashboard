...
def build_html(ctx: dict, kind: str, sample: bool = False,
    ...
    # ★★티켓번호는 **문서에 절대 안 적는다** (2026-09-03, 사용자 확정).
    #   지라 티켓은 **사내 문서번호**다. 정산서는 파트너사에 나가는 대외 문서라
    #   사내 번호가 실려서는 안 된다. 대외로 나가는 식별자는 표지 우상단의
    #   문서번호(`SB-SET-…`) 하나뿐이다.
    #   ★한 타이틀을 티켓 여러 장으로 정산하는 건(더윈드 = 폴라릿 31888 +
    #     스티커 프레임·포토카드 31552)에서 '무엇을 덮었나'가 필요하지만,
    #     그건 **화면에서만** 보여 준다(views/8 의 '티켓 N장을 한 장으로 정산해요').
    #   ★고치려 들기 전에 이 주석을 볼 것 — `ctx["tickets"][…]["ticket_keys"]` 가
    #     문서에 안 찍히는 건 **빠뜨린 게 아니라 그러기로 정한 것이다.**
