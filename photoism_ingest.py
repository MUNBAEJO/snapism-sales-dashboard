...
def main():
    ...
    try:
        ...
        # ★★원장(master_photoism.parquet)은 갱신됐는데 **집계본만 어제 것**으로
        #   남는다. 대시보드는 집계본을 읽으므로 최신인 척 옛 숫자를 보여 준다 —
        #   수집이 성공한 날에 화면만 안 움직이니 아무도 원인을 못 찾는다.
        #   경고 한 줄로 끝내지 말고 **종료코드로 알린다**(스케줄러가 실패로 본다).
        log(f"[실패] 집계 파일 갱신 실패 — 원장은 갱신됐지만 화면은 옛 숫자로 "
            f"남습니다. `python build_photoism_agg.py` 를 직접 돌려 주세요: {e}")
        sys.exit(1)
