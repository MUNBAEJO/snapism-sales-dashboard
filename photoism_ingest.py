...
def parse_excel(filepath: Path, country_code: str, config: dict) -> pd.DataFrame:
    ...
    # ★'결제일(지역)' 은 **수집만 하고 아직 안 쓴다** (2026-08-05).
    #   CMS 에 localPaymentDt 라는 현지시각 열이 있고(화면의 '결제일 (지역)',
    #   퀵사이트가 보여주는 값도 이것) 크롤러가 이제 같이 받아 온다.
    #   ★그런데 LA 매장 실측이 결제일 07-02 23:24 ↔ 지역 07-03 13:24 로 **14시간** 차다.
    #     날짜 기준을 이걸로 바꾸면 미국 거래가 거의 전부 하루씩 이동하고,
    #     이 열이 없는 과거 파일은 옛 기준으로 남아 **한 테이블에 두 기준이 섞인다.**
    #     정산 데이터에서 그건 위험해서, 전량 재수집 계획이 서기 전까지는 안 바꾼다.
    #   바꿀 때는 `결제일` → `결제일(지역)` 한 줄이면 되지만, 반드시 전 기간을
    #   같은 기준으로 다시 적재한 뒤에 해야 한다.
