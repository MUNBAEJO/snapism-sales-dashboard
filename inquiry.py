...
# ── 화면: 사이드바 ────────────────────────────────────────────────
# 창 안쪽 모양은 **여기서 못 박는다.** (창은 어느 화면에서 열어도 똑같아야 한다)
#
# ★★근본 원인은 고쳐졌다 (2026-09-02).
#   전엔 포토이즘·스내피즘·KPI목표가 화면 범위 없이 전역으로
#   `[data-testid="stSelectbox"]{max-width:240px}` 와 우측정렬을 걸어 둬서,
#   그 화면에서 창을 열면 분류 드롭다운이 쪼그라들어 오른쪽에 붙었다(실측 116px).
#   이제 세 화면 모두 `[data-testid="stMain"]` 으로 한정했고, `st.dialog` 는
#   stMain **밖** BODY 직속 포털이라 아예 안 닿는다(실측: 116px → 704px).
#
# ★그래도 아래 두 줄(max-width:none · display:block)은 **남겨 둔다.**
#   지금은 없어도 되지만, 나중에 누가 또 무범위 규칙을 넣으면 이 창부터 깨진다.
#   비용이 0 인 안전벨트다 — 지우지 말 것. 나머지 줄은 이 창 고유의 디자인이다
#   (폭 760px · 입력 높이 44px · 글자 14.5px).
_DIALOG_CSS = """
<style>
