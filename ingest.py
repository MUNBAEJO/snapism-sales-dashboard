...
def ingest():
    ...
    #     · (대시보드 8503 의 매일 09:30 '죽음' 은 이 압박과 무관하다 — 작업스케줄러
    #       snapism_restart 가 restart_dashboard.bat 로 8503 을 일부러 껐다 워치독이
    #       되살리는 **정기 재시작**이다. 워치독 로그의 매일 09:30 DOWN 이 그거다.
    #       메모리 압박에 의한 실제 화면 OOM 은 시각이 제각각이었고 로딩 복사 버그로
    #       따로 잡았다 — 09:30 을 OOM 으로 읽으면 안 된다.)
