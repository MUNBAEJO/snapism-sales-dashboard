...
def _sn_gubun(cats=None, camps=None) -> str:
    ...
    camps — 고른 **기획전**(원장 `카테고리` 열: 아티스트·캐릭터·반팔입고나와·
      후드입고나와·DIVE IN PHOTOISM·폴라릿…). **None 이면 안 거른다**.
    ★★왜 상품 축만으론 부족한가 (2026-09-04) — 두 계약이 **같은 상품을 둘 다 파는**
      경우가 있다. 누에라 2026-08 실측:
          아티스트     포토카드(IP) 1,059,599 · 와이드 스티커 327,033  ← CANDIP-29057
          반팔입고나와   포토카드(IP)   663,357 · 와이드 스티커 646,033  ← CANDIP-28379
      **기획전 × 상품이 격자**라 상품으로 자르면 두 계약이 섞인 채로 남는다.
      (리센느는 와이드↔반팔·미니↔메이생일 로 1:1 이라 상품 축으로 갈렸다.)
    ★두 축은 **AND** 로 걸린다 — 격자에서 한 칸만 집어낼 수 있어야 한다.
    """
    ...
    if camps is not None:
        if not camps:
            return "AND FALSE"
        out += (" AND COALESCE(NULLIF(trim(CAST(\"카테고리\" AS VARCHAR)), ''),"
                f" '기타') IN ({_sqlist(list(camps))})")
    return out
...
def country_detail(brand: str, titles: list[str], start: str, end: str,
                   rates: dict, frames=None, cats=None, camps=None) -> pd.DataFrame:
            ...
            df = con.execute(f"""
                WITH t AS (
                  ...
                  WHERE CAST("날짜" AS DATE) BETWEEN DATE '{start}' AND DATE '{end}'
                    AND "프레임 이름" IN ({_sqlist(titles)}) {_sn_gubun(cats, camps)}
...
def member_pivot(brand: str, titles: list[str], start: str, end: str,
                 ...
                 cats=None, camps=None) -> tuple[pd.DataFrame, pd.DataFrame]:
...
def price_table(brand: str, titles: list[str], start: str, end: str, frames=None,
                cats=None, camps=None) -> pd.DataFrame:
...
def cancel_amount(brand: str, titles: list[str], start: str, end: str,
                  rates: dict, frames=None, cats=None, camps=None) -> int:
...
def build_context(picks: dict, start: str, end: str, ip_name: str,
    ...
    # ★★**무엇을 좁혔는지 함께 남긴다** (2026-09-04).
    #   전엔 결과(`details`)만 저장하고 **그 결과를 만든 입력**은 안 남겼다. 그래서
    #   발행분을 재현할 수가 없었다 — 재발행하려면 어느 테마·멤버·상품·기획전을
    #   골랐는지 사람이 기억해야 했고, 감사도 못 했다.
    #   실제로 `PW 260801 반팔 입고 나와` 를 재계산하니 포토이즘이 +347,243,186 ·
    #   스내피즘이 +2,248,683 로 벌어졌는데, 틀린 게 아니라 **좁히기를 못 되살려서**였다.
    #   대외 문서라 "이 숫자가 어떻게 나왔나" 를 문서 스스로 답할 수 있어야 한다.
    # ★JSON 으로 저장되므로 튜플을 리스트로 편다.
    def _plain(v):
        if isinstance(v, (list, tuple)):
            return [_plain(x) for x in v]
        return v

    ctx["scope"] = {"frames": _plain(frames or {}) or None,
                    "cats": _plain(cats or {}) or None,
                    "camps": _plain(camps or {}) or None}
    for brand, sel in picks.items():
        ...
        ctx["pivots"][brand], ctx["pivots_rev"][brand] = member_pivot(
            brand, titles, start, end, rates, _tg, frames=_bf, cats=_bc, camps=_bp)
