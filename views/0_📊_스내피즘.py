            ...
            if not per.empty:
                with card("🎰 키오스크 1대당 매출 <span class='muted'>(팝업·렌탈 제외)</span>",
                    ...
                    html = (f'<div class="ntbl"><div class="ntr nth" style="{grid}">'
                            ...
                            '<span class="r">1대당 월건수</span><span>1위 대비</span></div>')
                    ...
                    helpbox("""
**키오스크 1대당 매출**
- **총 가동일** = 키오스크마다 그 기간에 실제로 돈 날짜를 모두 더한 값이에요. 3대가 30일씩 돌았으면 90이에요.
  - 조회기간과 **계약 기간이 겹치는 날짜만** 세요. 기간 중간에 계약이 시작되거나 해지되면 그만큼만 더해져요.
  - 왜 대수로 안 나누냐면 — 이번 달에 계약한 키오스크를 한 달 내내 있던 것처럼 세면 그 나라 대당 매출이 실제보다 낮게 나오거든요.
...
- **1위 대비** = 1등 국가를 100%로 둔 비율이에요. 위 '국가별 매출' 표의 **실결제 비중(전체 대비 점유율)과는 다른 값**이니 헷갈리지 마세요.
- 여기서 매출은 **실결제 + 쿠폰(정산금액)** 이에요. 실결제만 쓰면 전액 쿠폰으로 결제되는 국가(대만)가 1대당 0원이 돼요.
- **팝업·렌탈은 분자·분모 모두 제외**했어요. 며칠만 도는 행사 장비라 상시 매장과 섞으면 왜곡돼요.
- **기간 내 변동** `+N` = 신규 계약, `-N` = 실제 해지. 신규가 많은 나라는 총 가동일이 짧아 대당 매출이 눌려 보일 수 있어요.
