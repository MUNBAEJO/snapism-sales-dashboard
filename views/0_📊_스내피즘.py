...
# ── 매출 추이 스택 색 ────────────────────────────────────────────────────────────
# ★★2026-08-21: **1번(#6366f1)과 6번(#7c77ee) 색이 사실상 같은 색**
...
# ※PAL 자체는 다른 카드가 쓰고 있어 손대지 않는다 — 스택만 이 목록을 쓴다.
STACK_PAL = ["#6366f1", "#0f9d77", "#802d65", "#b45309", "#b8952e", "#38a3e8"]
...
def _load_data(v):
    ...
    # ★★결측을 먼저 빈 칸으로 만든다 (2026-08-21). 이 열은 nullable `string`
    #   dtype 이라 `astype(str)` 이 결측을 **'<NA>' 라는 글자**로 바꾼다.
    #   그래서 프레임 순위·IP 필터·내려받기에 `<NA>` 가 매출 8,352만원짜리
    #   IP 로 떠 있었다(31,730행). 아래 여러 곳이 "nan" 만 걸러 내고 있어서
    #   `<NA>` 는 그대로 통과했다 — 여기서 한 번에 없앤다.
    _s = df["프레임 이름"]
    if str(_s.dtype) == "category":
        _s = _s.astype("object")
    _s = (_s.fillna("").astype(str).str.strip()
...
# ★★취소 매출은 **양수 행만** 더한다 (2026-08-21).
#   취소된 거래는 원거래(+)와 반전행(−)이 **둘 다 '취소 여부' 플래그**를 달고
#   들어온다. 그냥 더하면 서로 상쇄돼 취소 규모가 실제보다 작게 나오고,
#   기간에 따라 **음수**로도 찍혔다(−19,500원). 전 기간 기준 단순합 16,748,800 vs
#   양수합 17,748,300(반전행 197개 · −999,500).
_cx = df[df["취소 여부"]]["KRW환산금액"]
cancel_krw = int(_cx[_cx > 0].sum())
...
- **취소 매출** = `취소 여부 = True` 거래 중 **금액이 양수인 행**의 `KRW환산금액` 합.
  **합계엔 포함하지 않아요**(환불분, 규모 참고용).
...
with tab_home:
    ...
    with card():
        ...
        def _trend():  # 프리셋 토글 → 매출 추이 차트
            ...
            # ★'한국' 열은 뺐다 — 한국 비중 배지를 없애면서 쓸 데가 없어졌다(2026-08-21).
            _g = _r.groupby("_d").agg(
                total=("정산금액", "sum"),
                실결제=("KRW환산금액", "sum"),
                쿠폰=("쿠폰KRW", "sum")).sort_index()
            ...
            trend_chart.render(st, _g, key="sn_trend", color="#4f46e5",
                               ...
                               stack_colors=STACK_PAL[:len(_cats)])
            helpbox("""
    **매출 추이**
    ...
    - **보기 4가지** — `올해`(1월~오늘, 월별 막대) · `최근 1년·월`(막대 12개, 해를 걸쳐서) · `12개월·주`(선 48점) · `최근 90일·일`(선 90점 + 7일 이동평균). **기본은 올해**예요.
      - 막대 **아래 숫자가 그 달 합계**예요.
    - ★**구성 고르기** — 바로 아래 줄에서 `전체` 또는 **상품 카테고리** 하나를 골라요.
      - `전체` 면 **한 색으로** 전체 흐름만 그려요. 카테고리별 금액은 **마우스를 올리면** 툴팁에 나와요(금액·비중).
      - 하나를 고르면 **그 카테고리만** 그 색으로 그려요.
      - ★잠깐 쌓아 봤다가 되돌렸어요 — 쌓으면 아래 층이 위를 밀어올려서 어느 층도 자기 값으로 안 읽혀요.
      - 고른 카테고리는 `12개월·주`·`최근 90일·일` 보기에도 그대로 적용돼요.
