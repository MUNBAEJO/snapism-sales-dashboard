...
# ── 정리 예정 안내 ────────────────────────────────────────────
# 이 페이지는 🧾 IP정산서 로 대체된다. 두 화면이 같은 IP에 **다른 금액**을 보여줄 수
# 있어(여기는 렌탈 제외 필터가 없고 환율 기준일도 다르다) 오래 두면 위험하다.
# 2026-06 정산을 새 페이지로 한 번 끝내고 숫자를 대조한 뒤 제거한다.
st.warning(
    "📢 **이 페이지는 곧 정리돼요.** 실제 정산은 사이드바의 **🧾 IP정산서** 에서 해주세요.\n\n"
    "여기는 스내피즘만 보고 렌탈·오리지널이 걸러지지 않아서, 같은 IP라도 "
    "정산서와 금액이 다를 수 있어요. 지금은 참고 조회용으로만 봐주세요.",
    icon="⚠️")

render_guide("settle")
st.caption("Snapism 브랜드 IP — 어드민 매출 × Jira RS율 정산 (참고 조회용)")
