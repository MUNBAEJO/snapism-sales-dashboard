...
# ── 구좌 색 ────────────────────────────────────────────────────────────
# ★★2026-08-21: **아티스트(#6366f1)와 오리지널(기본)(#7c77ee)이 사실상 같은 색**
#   이었다("색이 비슷해서 더 그렇다" — 사용자). 눈대중이 아니라 재 봤다:
#   두 색의 색차 ΔE 5.7 (정상 시야) — 구분 하한 15 의 3분의 1이다.
#   같이 드러난 것: **PICK(분홍)과 캐릭터(청록)이 적록색약에겐 ΔE 2.0** — 사실상
#   같은 색인데 하필 스택에서 서로 맞닿아 있었다(남성 약 8%).
#
#   그래서 두 자리를 옮겼다. 검증기 결과(전부 통과):
#     · 색약 최저 ΔE 8.9 (protan) · 정상 시야 최저 ΔE 15.8
#     · 금색·하늘색은 흰 배경 대비 3:1 미만이라 **라벨이 반드시 필요**하다
#       → 범례·막대 아래 금액·툴팁·구성 고르기가 그 몫을 한다.
#   덤: 오리지널 두 종이 갈색·금색으로 **같은 계열**이 돼 형제로 읽힌다.
PLUM, GOLD = "#802d65", "#b8952e"
_GUB_COLORS = {"아티스트": BRAND2, "캐릭터": TEAL, "PICK": PLUM,
               "오리지널(포토이즘)": AMBER, "오리지널(기본)": GOLD, "렌탈": SKY}
_GUB_EMOJI  = {"아티스트": "🎤", "캐릭터": "🧸", "PICK": "⭐",
               "오리지널(포토이즘)": "🎨", "오리지널(기본)": "🖼", "렌탈": "🏪"}
# 테마 비중 도넛용 색 — IP구분 색과 같은 계열에서 돌려 쓴다(화면 전체가 한 팔레트).
# 마지막 회색은 '기타 N개' 몫이다.
_GUB_CYCLE = [BRAND2, TEAL, PLUM, AMBER, GOLD, SKY, "#c7ccd6"]
...
# ※2026-08-21 현재 **부르는 곳이 없다** — 유일한 호출처였던 '타이틀 TOP 5' 가
#   IP명 합산(IP TOP 5)으로 바뀌었다. 타이틀 단위로 다시 보여줄 일이 있어
#   지우지 않고 남겨 둔다(같은 규칙을 두 번 만들지 않게).
...
- **포토이즘은 10일이 줄어요.** 취소는 **취소한 날짜에 음수 거래 한 줄**로 들어오고,
  원거래(5일)는 손대지 않아요. 그래서 **5일 매출은 그대로**고 10일 매출이 그만큼 깎여요.
  (원거래를 되짚는 키가 원본에 없어서 5일로 되돌릴 방법이 없어요.)
- **스내피즘은 반대로 5일이 줄어요** — 원거래에 `취소 여부` 플래그가 붙어 5일 매출에서 통째로 빠지고,
  10일엔 반전행이 들어오지만 그것도 취소라 매출에 안 잡혀요.
- ⚠️ **그래서 달을 넘긴 취소는 두 브랜드가 다르게 보여요.** 7월 판매가 8월에 취소되면
  포토이즘은 7월이 그대로·8월이 깎이고, 스내피즘은 7월이 깎여요. 월별 비교할 때 참고하세요.
""")
...
with tab_home:
    sec("1", "매출 동향",
        "잘 가고 있나? — <b>조회 기간을 안 따라요</b> · 오른쪽에서 기간, 아래에서 "
        "<b>구좌</b>를 골라요 "
...
- **보기 4가지** — `올해`(1월~오늘, 월별 막대) · `최근 1년·월`(막대 12개, 해를 걸쳐서) · `12개월·주`(선 48점) · `최근 90일·일`(선 90점 + 7일 이동평균). **기본은 올해**예요.
  - 막대 **아래 숫자가 그 달 합계**예요.
- ★**구성 고르기** — 바로 아래 줄에서 `전체 / 아티스트 / 캐릭터 / PICK / 오리지널 2종 / 렌탈` 을 골라요.
  - `전체` 면 **한 색으로** 전체 흐름만 그려요. 구좌별 금액은 **마우스를 올리면** 툴팁에 나와요(금액·비중).
  - 하나를 고르면 **그 구좌만** 그 구좌 색으로 그려요.
  - ★잠깐 구좌별로 색을 나눠 쌓아 봤다가 되돌렸어요 — 쌓으면 아래 층이 위를 통째로 밀어올려서
    **어느 층도 자기 값으로 안 읽혀요.** 색을 아무리 갈라도 안 풀리는 문제라, 정확히 볼 길은
    위 '구성 고르기' 로 열어 두고 그래프는 흐름만 보여 줘요.
  - 고른 구좌는 `12개월·주`·`최근 90일·일` 보기에도 그대로 적용돼요.
...
- **막대(연도) 보기만 구분별로 쌓아요.** 스택은 아래 계열이 위를 통째로 밀어올려서 각 구분을 눈으로는 못 읽어요 — 그래서 **선(주·일) 보기는 계열 하나**로 두고, 구분별 금액은 **툴팁에 숫자로** 보여줘요. 구성 비교는 바로 아래 '무엇이 매출을 만드나' 카드가 답해요.
...
""")
    ...
    # ★★2026-08-21 요청: '타이틀 TOP 5' → **IP TOP 5**.
    #   ① 회차를 합친다 — 같은 IP 가 `260721 SM ent`·`260811 SM ent` 처럼 회차마다
    #      갈려서 규모가 안 보였다(SM ent 는 합치면 1위인데 갈려서 2·4위로 보였다).
    #   ② **아티스트·캐릭터·PICK 만** 본다(요청). 오리지널 2종은 집계에 타이틀·IP명이
    #      아예 없고(프레임 단위라 그룹 폭증을 막으려 뺐다), 금액도 같은 기간 75억으로
    #      아티스트 전체(35억)의 두 배라 넣으면 이 카드가 통째로 오리지널이 된다.
    #      렌탈도 뺀다 — 행사 기간만 도는 별개 상품이다.
    #   ※ 회차를 합치면 `PW 260727 가나디` 도 `가나디` 로 합쳐진다(같은 IP명).
    _TOP5_GUB = ("아티스트", "캐릭터", "PICK")
    with card("🎞 IP TOP 5 <span class='muted'>(회차 합산 · 아티스트·캐릭터·PICK)</span>"):
        _tsrc2 = sales[sales["IP구분"].isin(_TOP5_GUB) & sales["IP명"].notna()
                       & (sales["IP명"] != "")]
        tr = (_tsrc2.groupby("IP명", observed=True)["매출액"].sum().rename("매출")
              .reset_index())
        tr = tr[tr["매출"] > 0]
        if not tr.empty:
            tr = tr.sort_values("매출", ascending=False).head(5)
            hbar_list(tr, "IP명", top=5)
        else:
            st.info("IP 데이터가 없어요.")
        helpbox("""
**IP TOP 5**
- `IP명`별 매출액 합 → 상위 5개. **회차(날짜)를 합친 값**이에요 — `260721 SM ent`,
  `260811 SM ent` 처럼 갈려 있던 걸 `SM ent` 한 줄로 봐요.
- **아티스트 · 캐릭터 · PICK 만** 세요(요청).
  - **오리지널 2종은 빠져요** — 집계에 IP명이 없고(프레임 단위), 금액도 아티스트 전체의
    두 배라 넣으면 이 카드가 통째로 오리지널이 돼요. 오리지널은 '구좌타입 분석' 탭에서 봐요.
  - **렌탈도 빠져요** — 행사 기간만 도는 별개 상품이에요.
- 회차를 합치니 `PW 260727 가나디` 처럼 앞 표식이 붙은 것도 같은 `가나디` 로 합쳐져요.
  회차·표식까지 나눠 보려면 '구좌타입 분석' 탭의 **구좌별 상세**에서 묶기를 '타이틀' 로 바꾸세요.
""")
...
""")
    with _n2:
        with card("🏬 국가별 매출 TOP 5 매장", key="scard-natsel"):
            ...
            #   기본이라 '한국 TOP 5' 만 보였고, 전 세계 상위 매장을 볼 방법이 없었다.
            ...
            if _cs:
                _pick = st.selectbox("국가", [_ALL_NAT] + _cs,
                                     key="ph_home_store_country", label_visibility="collapsed")
                _src = sales if _pick == _ALL_NAT else sales[sales["국가"] == _pick]
                _ss = (_src.groupby("매장 이름", observed=True)
                       .agg(매출=("매출액", "sum"), 건수=("건수", "sum"))
                       .reset_index().sort_values("매출", ascending=False).head(5))
                _ss = _ss[_ss["매출"] > 0]
                if not _ss.empty:
                    # 전체 보기일 땐 나라가 섞이니 국기를 붙인다 — 어느 나라 매장인지
                    # 이름만으론 알 수가 없다(TW_Zhong Shan 같은 이름도 있다).
                    if _pick == _ALL_NAT:
                        _ss = _ss.assign(표시=[flag_img(_v) + str(_n) for _n, _v
                                              in zip(_ss["매장 이름"],
                                                     _ss["매장 이름"].map(_nat_of(sales)))])
...
- 위 셀렉트박스에서 나라를 고르면 그 나라의 `매장 이름`별 매출액 합·건수 → 상위 5개로 바뀌어요.
""")
...
- **'○○ 대비'** = 이 표의 1위, 즉 **1대당 매출이 가장 높은 국가**를 100%로 둔 비율이에요. 헤더에 그 나라 이름이 그대로 나와요.
  ...
  - ⚠️ **표본이 적은 국가**(매출 발생 대수가 기준 미만)는 표 아래쪽으로 내려요. 매장 한 곳 성적이 그대로 국가 대표값이 돼서 1위로 튀거든요. 기준은 **최대 보유국의 1%**(최소 3대)라 나라 규모가 커지면 같이 올라가요. 100%와 헤더 국가명도 기준을 넘긴 나라에서만 잡아요. **그래서 표 중간에서 1대당 매출이 한 번 다시 올라가요** — 정렬이 틀린 게 아니라 위(기준 이상)·아래(표본 적음) 두 덩어리가 각각 내림차순이에요. (배지 표기는 요청으로 뺐어요.)
...
def _store_tab(sales, date_range, sel_countries):
    with card("🏬 매장 전체 순위"):
        ss = (sales.groupby("매장 이름", observed=True)
              .agg(매출=("매출액", "sum"), 건수=("건수", "sum")).reset_index())
        ss = ss[ss["매출"] > 0]
        # 국기(요청, 2026-08-21) — 30개국이 한 줄에 섞여 있어 이름만으론 어느 나라인지
        # 알 수가 없다. 이미지는 나라 수(≤30)만큼만 실제로 받아온다(같은 URL 재사용).
        _fl = ss["매장 이름"].map(_nat_of(sales))
