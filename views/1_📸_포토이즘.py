    ...
    with card():
        ...
        if _tsales.empty:
            ...
            trend_chart.render(st, _g, key="ph_trend", color="#4f46e5",
                               ...
                               # 월 막대만 구좌(IP구분)별로 색을 나눠 쌓는다.
                               # 색은 도넛·구분 카드와 같은 _GUB_COLORS 다.
...
- ★**구성 고르기** — 바로 아래 줄에서 `전체 / 아티스트 / 캐릭터 / PICK / 오리지널 2종 / 렌탈` 을 골라요.
  - `전체` 면 구좌별로 **색을 나눠 쌓고**, 하나를 고르면 **그 구좌만** 그려요.
  - 쌓아 놓으면 아래 층이 위를 밀어올려서 각 구좌를 눈으로는 못 읽어요(스택이라 어쩔 수 없어요).
    그래서 정확히 보려면 **골라서 보는 게 맞아요.** 마우스를 올리면 구분별 금액·비중도 나와요.
