...
""")
        ...
        if not _dev.empty and len(date_range) == 2 and "국가코드" in sales.columns:
            ...
            # 스4·포3.1: 분모를 '총 가동일'이 아니라 '가동 대수'로. 조회기간이 30일이 아닐 때만
            ...
            if not per.empty:
                with card("🎰 키오스크 1대당 매출 <span class='muted'>(렌탈·팝업 제외)</span>",
                    ...
                    grid = ("grid-template-columns:1.4fr .7fr .95fr 1.2fr "
                            ".9fr 1.15fr")
                    html = (f'<div class="ntbl"><div class="ntr nth" style="{grid}">'
                    ...
                    helpbox("""
**키오스크 1대당 매출**
- **1대당 월매출** = 조회기간 **실제** 매출 ÷ **가동 대수**. **예상치가 아니에요.** 조회기간이 30일이면 그대로 한 달 실적이고, 30일이 아니면 30일치로 환산해요(7일만 보면 ×30/7). 월건수도 같은 방식이에요.
