...
def _ticket_box(brand: str):
    ...
    chosen, tmap = [], {}
    extra_tks = []           # 대표 외에 '같이 담은' 티켓들 — 화면에만 보인다
    for t in order:
        cands = claimed[t]
        fixed = tt[t][0]
        ...
        #   한 장만 고를 수 있으면 나머지 티켓을 정산에 물렸다고 **기록할 데가 없다.**
        #   ★매출은 **타이틀 기준**이라 티켓을 더 골라도 **금액은 안 변한다.**
        #     바뀌는 건 화면에 적히는 티켓번호와 확정 저장 대상뿐이다 —
        #     티켓번호는 사내 번호라 정산서에는 안 실린다.
        ...
        picked = st.multiselect(
            ...
            help="이 타이틀 매출을 어느 티켓으로 정산할지 골라요. 상품별로 티켓이 "
                 ...
                 "금액은 그대로예요. 티켓번호는 사내 번호라 **정산서에는 안 나가고** "
                 "화면에서만 보여요. 계약 티켓이 아니라 실제 상품 티켓을 고르세요.")
        if on:
            chosen.append(t)
            if picked:
                # ★확정 저장·요율은 **대표 한 장**(맨 앞)을 따른다. 요율이 하나라는
                #   전제이고(사용자 확정), 매핑 저장소도 타이틀당 티켓 하나를 쥔다.
                #   나머지 티켓은 **화면에만** 남는다 — 티켓번호는 사내 문서번호라
                #   대외 문서(정산서)에 싣지 않는다(settlement_pdf 의 같은 주석 참고).
                tmap[t] = picked[0]
