...
# ── 스내피즘 정산 대상 판정 ────────────────────────────────────────────────
# ★★★2026-09-03: 스내피즘은 **카테고리 허용목록을 버리고 '프레임 이름이 있는가'**
#   로 판정한다(사용자 지정). 위 `SETTLE_GUBUN` 은 이제 **포토이즘 전용**이다.
#
# 왜 바꿨나 — 스내피즘의 `카테고리` 열에는 IP구분만 들어오는 게 아니라
#   **기획전 이름이 그대로 들어온다**: `반팔입고나와` · `후드입고나와` ·
#   `DIVE IN PHOTOISM` · `5월의 놀이터` · `후드기획전` · `COMMING SOON`.
#   허용목록에 없으니 통째로 탈락했고, 그래서 **실제로 판 IP가 정산 후보에
#   아예 안 나타났다** — 허남준(`CANDIP-32742`)을 발행하려는데
#   "이 기간에 그 티켓으로 잡히는 매출이 없어요" 가 떴다(2026-09-03 신고).
#   2026년 규모: 32,338건 · **국내 ₩140,966,000**. 리센느·엔플라잉·제로베이스원·
#   프로미스나인·크래비티·허남준·비투비·롱샷·로이킴·원위·이민혁(HUTA) 등.
#   ★이미 발행된 문서 2건이 실제로 덜 지급됐다(비투비컴퍼니 ₩2,701,000 ·
#     fromis_9 ₩8,410,000 · 국내 기준). **재발행은 안 한다 — 수동 정산으로
#     보냈다**(2026-09-03 사용자 확정).
#
# ★이건 '재발'이 아니라 **결정을 뒤집는 것**이다. `a6087c0`(2026-08-04)이
#   "같은 IP라도 카테고리가 다르면 정산 조건이 다르다" 를 근거로 이 셋을 일부러
#   뺐다. 그 커밋의 취지는 **후보 목록과 정산 집계를 일치시키는 것**이었고
#   (전엔 화면에 없던 매출이 정산액에 조용히 더해졌다), 그 일치는 지금도 지킨다 —
#   아래 한 식을 두 곳이 같이 쓴다. 바뀐 건 '무엇을 대상으로 보는가' 뿐이다.
#
# 왜 프레임 이름인가 — 스내피즘에서 IP를 가리키는 열은 `프레임 이름` 하나뿐이다
#   (`IP 이름` 열은 2026-08 기준 97% 가 비어 있다). 프레임이 있으면 그 IP 상품이고,
#   없으면 커스텀·비IP 상품이다. **새 기획전이 생겨도 다시는 조용히 빠지지 않는다** —
#   허용목록은 새 값이 나타나면 말없이 떨어뜨리고, 정산서에서 그 방향의 실패는
#   언제나 **덜 지급**이다.
# 검증(2026년 전체): 이 규칙으로 **빠지는 행 0건**, 들어오는 것만 32,338건.
SNAPISM_SETTLE_SQL = "COALESCE(TRIM(CAST(\"프레임 이름\" AS VARCHAR)), '') <> ''"
